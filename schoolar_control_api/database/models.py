...
from sqlalchemy import JSON, Boolean, Date, DateTime, Integer, Numeric, String, ForeignKey, Column, Table, CheckConstraint, Text
...
class Degree(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint('LENGTH(description) > 3', name='check_degree_description_length'),
    )
...
class Teacher(Base):
    ...
    __table_args__ = (
        CheckConstraint('LENGTH(specialization) > 3', name='check_specialization_length'),
    )
...
class Student(Base):
    ...
    key_registration: Mapped[str] = mapped_column(String(20), unique=True, nullable=False)
    created_at: Mapped[datetime] = mapped_column(nullable=False, default=datetime.utcnow)
    updated_at: Mapped[datetime] = mapped_column(nullable=False, default=datetime.utcnow, onupdate=datetime.utcnow)
    deleted_at: Mapped[Optional[datetime]] = mapped_column(nullable=True)
...
class Course(Base):
    ...
    period_id: Mapped[int] = mapped_column(ForeignKey('academic_periods.id'), nullable=False)
    ...
    units: Mapped[List['Unit']] = relationship(back_populates='course', order_by='Unit.order_index')
    evaluation_components: Mapped[List['EvaluationComponent']] = relationship(back_populates='course')
...
class CourseEnrollment(Base):
    ...
    enrollment_date: Mapped[datetime] = mapped_column(nullable=False, default=datetime.utcnow)
    status: Mapped[str] = mapped_column(String(50), default='active', nullable=False)
    created_at: Mapped[datetime] = mapped_column(nullable=False, default=datetime.utcnow)
    updated_at: Mapped[datetime] = mapped_column(nullable=False, default=datetime.utcnow, onupdate=datetime.utcnow)
...
class Platform(Base):
    ...
    components: Mapped[List['EvaluationComponent']] = relationship(back_populates='platform')
...
class Task(Base):
    ...
    course_id: Mapped[int] = mapped_column(ForeignKey('courses.id'), nullable=False)
    unit_id: Mapped[int] = mapped_column(ForeignKey('units.id'), nullable=False)
    component_id: Mapped[int] = mapped_column(ForeignKey('evaluation_components.id'), nullable=False)
    platform_id: Mapped[Optional[int]] = mapped_column(ForeignKey('platforms.id'), nullable=True)
    name: Mapped[str] = mapped_column(String(255), nullable=False)
    description: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
